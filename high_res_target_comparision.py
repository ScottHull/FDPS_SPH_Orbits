...
low_df = pd.read_csv(low_res_path, skiprows=2, header=None)
low_df['radius'] = [(i ** 2 + j ** 2 + k ** 2) ** (1 / 2) / (6371 * 1000) for i, j, k in zip(low_df[3], low_df[4], low_df[5])]
low_silicate = low_df[low_df['tag'] == 0]
low_iron = low_df[low_df['tag'] == 1]

high_df = pd.read_csv(high_res_path, skiprows=2, header=None)
high_df['radius'] = [(i ** 2 + j ** 2 + k ** 2) ** (1 / 2) / (6371 * 1000) for i, j, k in zip(high_df[3], high_df[4], high_df[5])]
high_silicate = high_df[high_df['tag'] == 0]
high_iron = high_df[high_df['tag'] == 1]
