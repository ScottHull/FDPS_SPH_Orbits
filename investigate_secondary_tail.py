...
def get_secondary_and_tail():
    ...
    for output_name, title in zip(sims, titles):
        ...
        si_ang_mom = si['mass'] * np.array([np.cross(i, j) for i, j in zip(si_position_vector, si_velocity_vector)])
        tail_ang_mom = tail['mass'] * np.array([np.cross(i, j) for i, j in zip(tail_position_vector, tail_velocity_vector)])
